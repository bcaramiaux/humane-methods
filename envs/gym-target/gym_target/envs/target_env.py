...
class TargetEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = Space(self.n_dim)

        high = np.ones((self.n_dim,))
        low = np.zeros((self.n_dim,))
    ...
    def actions(self, s):
        ...
        for i in range(self.n_dim):
            if s[i] + 1/self.n_quant < 1.0:
                valid_acts.append(i)
        for i in range(self.n_dim):
            if s[i] - 1/self.n_quant > 0.0:
    ...
    def step(self, action):
        s = self.state
        dist = np.sqrt(np.sum([(s[i] - self.goal[i])**2 for i in range(self.n_dim)]))/self.n_dim
        if len(np.where(s[:] > 1.0)[0]): 
            reward = 0.0
            for i in np.where(s[:] > 1.0)[0]:
                reward += (1.0 - s[i]) * 1000.0
        elif len(np.where(s[:] < 0.0)[0]):
            reward = 0.0
        ...
        if action < self.n_dim:
            next_s = np.copy(s)
            next_s[ action] += 1/self.n_quant
        elif action >= self.n_dim:
            next_s = np.copy(s)
            next_s[ action - self.n_dim] -= 1/self.n_quant
